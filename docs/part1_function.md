...
* 콜백 함수를 인자로 넘기는 대신 Promise 객체 생성 후 리턴
* 호출부에서는 리턴받은 Promise 객체에 `then()` 메소드를 호출하여 실행할 로직 넘겨줌

즉, 콜백 함수를 통해 비동기 처리하던 기존 코드와의 다른 점은 함수를 호출하면 Promise 타입의 결과값이 리턴되고, 이 결과값을 가지고 다음 수행할 작업을 진행한다는 것이다. 따라서 비동기 처리 코드임에도 불구하고 마치 동기 처리 코드처럼 코드가 읽히기 때문에 마치 동기 처리 코드와 같은 느낌을 받을 수 있다.

[추후 작성](https://www.daleseo.com/js-async-promise/).
