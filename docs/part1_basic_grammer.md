...
1. [요약](#요약)
	...
	* [자료형](#자료형)
		...
		* [연산자](#연산자)
	* [변수](#변수)
		* [let 키워드와 const 키워드](#let-키워드와-const-키워드)
2. [참고](#참고)
...
* [자바스크립트에서 **불 자료형 변환이** false로 간주되는 것들](https://studymake.tistory.com/484)
	* 불 자료형 변환(Boolean() 함수 사용)을 하지 않은 것은 결과가 다를 수 있음
* 숫자와 문자열 자료형 변환
	* [추후 추가]()
...
#### let 키워드와 const 키워드
ECMAScript 5까지는 `식별자에 값을 넣어 활용하기 위한 기능`이 `var 키워드를 사용한 변수`밖에 없었다. 그런데 ECMAScript 6부터는 `let 키워드를 사용한 변수`, `const 키워드를 사용한 상수`라는 개념이 추가되었다.
...
상수는 선언한 이후에 값을 변경할 수 없으므로, 선언할 때 반드시 값을 함께 넣어줘야 한다(cf. 자바). ECMAScript 5까지는 var 키워드를 사용한 변수밖에 없었는데, ECMAScript 6부터 상수라는 개념이 생긴 이유는 성능 향상 때문이다. 또한 변수에 관해서는 아래 설명한다.

* var 키워드로 선언한 변수와 let 키워드로 선언한 변수의 차이
	* 특정 스코프의 변수(var)를 외부의 다른 스코프에서 사용
		* 일반적으로 스코프 내부에 선언한 해당 변수를 생성한 이후 스코프 내부에서 사용하고, 스코프를 벗어날 때는 제거해야 메모리 등의 시스템 자원을 적절하게 이용할 수 있다.
		* 자바스크립트의 var 키워드는 그렇지 못한 반면 let 키워드는 자원을 적절하게 이용할 수 있도록 특정 스코프 내부에서만 변수를 사용할 수 있도록 함
	* var 키워드의 비동기 함수 문제
